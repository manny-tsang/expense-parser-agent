...
- `match_merchant_category_pattern(self, merchant_name: str) -> Optional[int]`: Evaluates an unmapped merchant string against existing mapped merchants in SQLite using SQL substring matching (`WHERE category_id != 1 AND UPPER(?) LIKE '%' || UPPER(merchant_name) || '%'`). Returns the mapped `category_id` of the matching pattern if found, otherwise `None`.
